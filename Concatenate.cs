...
namespace SanadDiP
{
    ...
    public class Concatenate // One method for both options
    {
            ...
            unsafe
            {
                ...
                if (horizontal)
                {
                    for (int i = 0; i < bigH; i++) // looping through whole image height
                    {
                        ...
                        {
                            if (i >= h1)
                                *newPixel = 255; // if image height is shorter than whole concattenated image height make it white pixel
                            else
                            {
                                *newPixel = *pixel1; // copy first image pixel
                                pixel1++;
                            }
                        ...
                        for (int j = 0; j < w2; j++, newPixel++) // second image copying row values
                        {
                            ...
                            {
                                *newPixel = *pixel2; // copy second image pixel
                                pixel2++;
                            }
                        }
                ...
                else                                // not horizontal
                {
                    for (int i = 0; i < h1; i++)    // copy whole first image into concatenated image
                    {
                        ...
                        {
                            if (j >= w1)
                                *newPixel = 255; // if image width is shorter than whole concattenated image width make it white pixel
                            else
                            {
                                *newPixel = *pixel1; // copy first image pixel
                                pixel1++;
        ...
        {                                                                       // Horizontal concatenation means width = W1 + W2, height = Max(H1, H2)            
            ...
            if (!horizontal)
                concat.RotateFlip(RotateFlipType.Rotate270FlipNone);
