...
namespace SanadDiP
{
    ...
    {
        private static int[,] sqr = new int[,] { { 1, 1, 1 }, { 1, 1, 1 }, { 1, 1, 1 } };
        ...
        {
            Bitmap newB = ImageAlteration.AddBorder(b, 0);
            ...
            int t1 = strel[0, 0], t2 = strel[0, 1], t3 = strel[0, 2], 
                m1 = strel[1, 0], m2 = strel[1, 1], m3 = strel[1, 2], 
                b1 = strel[2, 0], b2 = strel[2, 1], b3 = strel[2, 2]; 
            ...
            unsafe
            {
                byte* top = (byte*)bmd1.Scan0.ToPointer();
                byte* mid = top + stride1;
                byte* bot = mid + stride1;
                byte* pixel = (byte*)bmd2.Scan0.ToPointer();

                for (int y = 0; y < bH; y++, top+=offSet1, mid+=offSet1, bot+=offSet1, pixel+=offSet2)
                {
                    for (int x = 0; x < bW; x++, pixel++, top++, mid++, bot++)
                    {
                        if (t1 > 0.5 && top[0] > 128) 
                            pixel[0] = 255;
                        else
        ...
        {
            Bitmap newB = ImageAlteration.AddBorder(b, 255);
            int bW = b.Width, bH = b.Height, bW2 = bW+2, bH2 = bH+2;
            PixelFormat pF = PixelFormat.Format8bppIndexed;
            BitmapData bmd1 = newB.LockBits(new Rectangle(0, 0, bW2, bH2), ImageLockMode.ReadOnly, pF);
